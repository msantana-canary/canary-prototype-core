...
import { Thread, Message, LinkedReservation, MessageChannel } from '@/lib/products/messaging/types';
...
export function ThreadView({
  ...
  // Email left the messaging surface (SJ pivot 2026-06-11) — it lives in the
  // top-level Email tab. Channel tabs left too (2026-06-12): instant channels
  // are already unified chronologically, matching production. The composer
  // targets wherever the conversation last happened; clicking any bubble
  // switches the send channel to that bubble's.
  const instantMessages = useMemo(
    () => messages.filter((m) => m.channel !== 'Email'),
    [messages]
  );

  const lastInstantChannel: MessageChannel = useMemo(() => {
    const withChannel = instantMessages.filter((m) => m.channel);
    return withChannel[withChannel.length - 1]?.channel || 'SMS';
  }, [instantMessages]);

  useEffect(() => {
    if (selectedChannel !== lastInstantChannel) {
      onChannelChange(lastInstantChannel);
    }
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white relative">
      ...
      {/* Hairline between header and feed (channel tabs removed 2026-06-12 —
          instant channels are one chronological stream, like production) */}
      <div className="border-b border-gray-200 bg-white" />

      {/* Messages — all instant channels, chronological */}
      <MessageFeed
      ...
      {/* Composer — targets the active channel; click a bubble to switch */}
      <div className="-mt-5">
        <MessageComposer
