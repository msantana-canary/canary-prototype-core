...
## Single Stream (comparison variant — the last unexplored cell)
**Decision:** Third "Channel Tabs" option: **Single stream** — no tabs at all; every channel *including email* in one chronological thread. Email bubbles carry their subject inline as a muted label ("Guest Journey — [subject]" for automated sends). Click any bubble to set the reply context: email bubble → reply target with "Replying to: [subject]" + Reply button (that thread's bubbles ring blue); non-email bubble → send channel switches. Entry defaults to wherever the conversation last happened, email included. Sent email replies now carry the reply target's emailThreadId (store fix — they previously landed subject-less, which also broke grouping in the unified email view).
**Why:** Miguel/Rachel sync 2026-06-10: *"I will make another version to see what happens if emails are interspersed within the [thread], see where that goes."* It was the only arrangement never prototyped — every other variant protects the email split. This is the Akia pattern Rachel referenced (emails inline in one long thread, guest messages marked) and the dominant industry interleave from the June 7 Mobbin research. Built so tomorrow's group can judge it live instead of abstractly; also the cleanest way to *demonstrate* why email-as-separate wins (the Honda Civic in the BMW lane), if it does.
**Rejected:** Leading with it tomorrow (two-tab is the focus; this is a float-if-aligned option). Subject group headers in the mixed stream (would fragment constantly when channels alternate — the "switch, switch, switch" problem; inline per-bubble labels instead).
**Open:** Whether seeing it live kills it or revives it. The unified *email* view's own open question still stands: when email threads genuinely interleave in time, does chronological-by-message hold up vs. grouping per-thread — current mock data (Fatima) has cleanly sequential threads, so a stress-test guest with interleaved email threads is the natural next probe.
