...
## Status (2026-06-12) — 🟢 SJ APPROVED the overlay. Now Slack-minimalism UI polish with David Champeil.
**SJ signed off on the bottom-left quick-fire overlay (D23) — "I'm aligned on functionality."** The 2-week saga is resolved; remaining work is UI polish (make it Slack: compact, no dividers/bold/avatars, distinct color, bigger panel), not direction. Canary Support → top-right; gate launch to non-Wyndham first. Pairing with David Champeil on the aesthetic. See **D23**. (Arc: D19 push → D22 SJ-reversed-to-overlay → **D23 SJ-approved-overlay + Slack-polish**.)

> **Prior (2026-06-10, resolved by D23):** ⚠️ DIRECTION REVERSED by SJ: PUSH is dead, OVERLAY (quick-fire) is the call
...
### D23 — SJ APPROVED the overlay; refine to Slack-level minimalism (resolves the saga)
- **Decision (2026-06-12 SJ review):** SJ signed off — "I'm aligned on functionality." Ship the **bottom-left quick-fire overlay** (D22 direction), refined to feel like **Slack, not our guest-messaging product** (minimalist/compact/"terminal-like"). Remaining work = UI polish, not direction. **No scrim** — the opaque, drop-shadowed contained card won; **covering the sidebar is acceptable** ("tap = intent is chat; the nav can hide + come back").
- **Polish punch-list (SJ's notes):** Slack-tight sidebar (~15–20px line items, NO dividers, NO bold, NO avatars, text-only, much narrower; kill the preview line) · structure = **Departments → Staff → (DMs later)** · **rename "Groups"→"Departments"** · panel gets **bigger** (top line → bottom of the left region) · **Canary Support → top-right** help button (off the bottom-left) · **distinct color** for the Team Chat header/bg (lighter blue, NOT selected-state blue; exact TBD) · **remove** the persistent seen-by icon (read receipts maybe hover, V1) · **no new design system** (color + spacing deviations only).
- **Scope flag (OPEN):** SJ wants the Slack layout (Departments + Staff) but said "**v0 won't have DMs**, just lay it out so DMs slide underneath later." Reconcile w/ PRD ("no 1:1 DMs"): v0 *shows* staff without DM, or do DMs enter v0? → Jake.
- **Rollout:** moving Zendesk support may draw **Wyndham** UX pushback → **gate launch to non-Wyndham first** (talk to Gareth). SJ: "we can launch team chat not to Wyndham."
- **Collaborator:** **David Champeil** (strong UI eye) pairs with Miguel on the minimalist polish.
- **Rejected/settled:** the off-to-side hedge (SJ: cover the sidebar, make it bigger) · the persistent always-on seen-by icon (removed) · a new design system (no) · full-screen now (v1 maybe).
